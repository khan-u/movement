...
def process_single_validation(args):  # noqa: C901
    ...
    with open(os.devnull, "w") as devnull:
        ...
        try:
            ...
            # Store per-keypoint variance arrays
            lat_std = val.get("lateral_std")
            if lat_std is not None:
                rec["lateral_std"] = lat_std.tolist()
            else:
                rec["lateral_std"] = None
            long_std = val.get("longitudinal_std")
            if long_std is not None:
                rec["longitudinal_std"] = long_std.tolist()
            else:
                rec["longitudinal_std"] = None
...
def save_to_h5(  # noqa: C901
    ...
    with h5py.File(output_path, "w") as f:
        ...
        # Save avg_skeleton, PC1, vel_projs, and variance arrays
        ...
        lat_std_grp = f.create_group("lateral_std")
        long_std_grp = f.create_group("longitudinal_std")
        ...
        for r in all_results:
            ...
            if pd["lateral_std"] is None and r.get("lateral_std") is not None:
                pd["lateral_std"] = r.get("lateral_std")
            long_std = r.get("longitudinal_std")
            if pd["longitudinal_std"] is None and long_std is not None:
                pd["longitudinal_std"] = long_std
        ...
        for (file_stem, individual), data in pair_data.items():
            ...
            if file_stem not in skel_grp:
                ...
                lat_std_grp.create_group(file_stem)
                long_std_grp.create_group(file_stem)
            ...
            if data["lateral_std"] is not None:
                arr = np.array(data["lateral_std"], dtype=np.float64)
                lat_std_grp[file_stem].create_dataset(individual, data=arr)
            if data["longitudinal_std"] is not None:
                arr = np.array(data["longitudinal_std"], dtype=np.float64)
                long_std_grp[file_stem].create_dataset(individual, data=arr)
...
def plot_validation_results(analysis_data, output_path):  # noqa: C901
    ...
    keypoints_by_file = analysis_data.get("keypoints", {})
    skeleton_data = []
        ...
        skeleton_data.append(
            {
                ...
                "keypoints": keypoints_by_file.get(f, []),
            }
        )
    ...
    if n_skeletons > 0:
        ...
        for idx, skel in enumerate(skeleton_data):
            ...
            if from_idx >= 0 and to_idx >= 0:
                ...
                # Add node names below [#, #] with same color coding
                keypoints = skel.get("keypoints", [])
                from_ok = from_idx < len(keypoints)
                to_ok = to_idx < len(keypoints)
                if keypoints and from_ok and to_ok:
                    from_name = keypoints[from_idx]
                    to_name = keypoints[to_idx]
                    name_y = pair_y - 0.22  # Below indices

                    # Format: "[ " + from_name + " , " + to_name + " ]"
                    name_total = 2 + len(from_name) + 3 + len(to_name) + 2
                    name_start_x = x_offset - (name_total * char_width) / 2

                    # Draw each part with color coding
                    npos = name_start_x
                    txt_args = {"ha": "left", "va": "top", "fontsize": 10}
                    txt_args["fontweight"] = "bold"
                    txt_args["clip_on"] = False
                    ax1.text(npos, name_y, "[ ", color="black", **txt_args)
                    npos += char_width * 2
                    ax1.text(
                        npos,
                        name_y,
                    ...
                    npos += char_width * len(from_name)
                    ax1.text(npos, name_y, " , ", color="black", **txt_args)
                    npos += char_width * 3
                    ax1.text(
                        npos, name_y, to_name, color=to_num_color, **txt_args
                    )
                    npos += char_width * len(to_name)
                    ax1.text(npos, name_y, "  ]", color="black", **txt_args)
        ...
        ax1.set_ylim(-2.1, 1.2)  # Extended for node names
    ...
    # Add arrow description at top left (black text)
    fig.text(
        0.01,
        0.99,
    ...
    # Step 2 shows surviving pair counts directly.
    # Step 3 classifies survivors as distal or proximal (no elimination).
    step1_counts = []
    step2_counts = []
    step3_distal = []
    step3_proximal = []
    selection_types = []  # "distal" or "proximal" for each file
    ...
    for f in files:
        ...
        n_s3p = n_s2 - n_s3d  # Proximal = survivors minus distal

        s1_pairs = n_s1 * (n_s1 - 1) // 2 if n_s1 >= 2 else 0
        step1_counts.append(s1_pairs)
        step2_counts.append(n_s2)
        step3_distal.append(n_s3d)
        step3_proximal.append(n_s3p)

        # Get selection type for this file
        sug = suggested_analysis.get(f, {})
        selection_types.append(sug.get("suggested_type", ""))

    x_pos = np.arange(n_files)
    bar_width = 0.25

    # Hatch patterns distinguish steps
    step_hatches = ["--", "||"]

    # Draw Step 1 and Step 2 bars (no labels - only Step 3 gets labels)
    for j, (counts, hatch) in enumerate(
        zip([step1_counts, step2_counts], step_hatches, strict=True)
    ):
    ...
    # Draw Step 3 as stacked bar (distal bottom, proximal top)
    step3_offset = 1 * bar_width  # Position for Step 3
    for i in range(n_files):
        c = colors[i]
        bx = x_pos[i] + step3_offset
        n_distal = step3_distal[i]
        n_proximal = step3_proximal[i]
        sel_type = selection_types[i]

        # Distal portion (bottom, solid hatch)
        fc_distal = (c[0], c[1], c[2], 0.50)  # Darker for distal
        ax4.bar(
            bx,
            n_distal,
            bar_width,
        ...
        # Proximal portion (top, lighter)
        fc_proximal = (c[0], c[1], c[2], 0.20)  # Lighter for proximal
        ax4.bar(
            bx,
            n_proximal,
            bar_width,
        ...
        total = n_distal + n_proximal
        ax4.text(
            bx,
            total + 0.5,
            f"{n_distal}D/{n_proximal}P",
            ha="center",
            va="bottom",
            fontsize=7,
            fontweight="bold",
        ...
        # Selection indicator (star) on the selected segment
        if sel_type == "distal" and n_distal > 0:
            marker_y = n_distal / 2
            ax4.plot(
                bx,
                marker_y,
        ...
        elif sel_type == "proximal" and n_proximal > 0:
            marker_y = n_distal + n_proximal / 2
            ax4.plot(
                bx,
                marker_y,
                marker="*",
    ...
    legend_patches = [
        ...
        mpatches.Patch(
            ...
            linewidth=0.5,
            hatch="||",
            label="Step 2: Opposite-Sides",
        ),
        mpatches.Patch(
            facecolor=(0.5, 0.5, 0.5, 0.50),
            edgecolor="black",
            linewidth=0.5,
            hatch="xx",
            label="Step 3: Distal",
        ...
        mpatches.Patch(
            facecolor=(0.5, 0.5, 0.5, 0.20),
            edgecolor="black",
            linewidth=0.5,
            hatch="..",
            label="Step 3: Proximal",
        ),
        Line2D(
            [0],
            [0],
    ...
    step3_totals = [
        d + p for d, p in zip(step3_distal, step3_proximal, strict=True)
    ]
    max_count = max(
        max(step1_counts) if step1_counts else 1,
        max(step2_counts) if step2_counts else 1,
        max(step3_totals) if step3_totals else 1,
    )
    ...
    ax4.set_title(
        "Filter Cascade: Steps 1-2 Filter, Step 3 Classifies",
        fontsize=12,
        fontweight="bold",
        color=text_color,
    )
...
def create_individual_plot(  # noqa: C901
    ...
    if n_keypoints >= 20:
        text_x, text_ha = 0.78, "center"  # beneath tile 4
        y_line1, y_line2 = 0.032, 0.004  # moved downward
    else:
        text_x, text_ha = 0.5, "center"
        y_line1, y_line2 = 0.045, 0.012
